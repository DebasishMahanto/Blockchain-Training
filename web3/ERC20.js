...
let aAccounts
let ERC20Contract;
const contractAddress = "0xbDdf9Ea9DdE7808e80B64c385E7b3b367FB62F05";
const contractAbi = [
    {
        "inputs": [
            {
...
async function tokenDetails() {
    ...
    getName.innerText = "Token Name: " + await ERC20Contract.methods.name().call();

    var getSymbol = document.getElementById("tokenSymbol");
    getSymbol.innerText = "Token Symbol: " + await ERC20Contract.methods.symbol().call();

    var getTotalSupply = document.getElementById("totalSupply");
    getTotalSupply.innerText = "Total Supply in wei: " + await ERC20Contract.methods.totalSupply().call();

    var getBalance = document.getElementById("balanceId");
    getBalance.innerText = "Balance in wei: " + await ERC20Contract.methods.balanceOf(accounts[0]).call();
...
async function mintToken() {
    ...
    try {
        ...
        await ERC20Contract.methods.mint(amount).send({ from: accounts[0] });
...
async function transferToken() {
    ...
    try {
        ...
        nGasUsed = await ERC20Contract.methods
            ...
            .estimateGas({ from: accounts[0] }, function () { });
        console.log(nGasUsed);
        await ERC20Contract.methods.transfer(receiverAddress, amount).send({ from: accounts[0] });
...
async function approveToken() {
    try {
        ...
        console.log(nGasUsed);
        await ERC20Contract.methods.approve(spenderAddress, amount).send({ from: aAccounts[0] });
        console.log("token transferd");
...
async function aTransferFrom() {
    try {
        ...
        const receiverAddress = await document.getElementById('IDreceiverAdd').value;
        ...
        if (senderAddress == aAccounts[0]) {
            nGasUsed = await ERC20Contract.methods
                ...
                .estimateGas({ from: aAccounts[0] }, function () { });
            console.log(nGasUsed);
            await ERC20Contract.methods.transfer(receiverAddress, amount).send({ from: aAccounts[0] });
            console.log("token transferd");
            tokenDetails();
            ...
            nGasUsed = await ERC20Contract.methods
                ...
                .estimateGas({ from: aAccounts[0] }, function () { });
            console.log(nGasUsed);
            await ERC20Contract.methods.transferFrom(senderAddress, receiverAddress, amount).send({ from: aAccounts[0] });
            console.log("token transferd");
            tokenDetails();
        }
...
async function allowanceCheck() {
    ...
    try {
        ...
        let amount = await ERC20Contract.methods
            ...
            .call();
        console.log("allowance:", amount, "from:", tokenOwnerAddress, "to:", approvalAddress);
    } catch (error) {
        alert(error.message);
