...
contract MarketPlace {
    ...
    mapping(bool => tokenOnSale[]) private myTokenOnSale;
    ...
    function purchase(
        ...
        if (tokenOnSaleInfo[_isERC721][_tokenId][_sellerAddress].isERC721) {
            ...
            // myTokenOnSale[msg.sender];
            removeMyToken(_tokenId, _isERC721, _sellerAddress);
        } else {
            IToken1155(erc1155).safeTransferFrom(
                tokenOnSaleInfo[_isERC721][_tokenId][_sellerAddress].seller,
                msg.sender,
                _tokenId,
                _quantity
            ...
            tokenOnSaleInfo[_isERC721][_tokenId][_sellerAddress]
                .quantity -= _quantity;
            // myTokenOnSale[_sellerAddress][].quantity-=_quantity;
            for (uint256 i; i < myTokenOnSale[_isERC721].length; i++) {
                if (myTokenOnSale[_isERC721][i].tokenId == _tokenId) {
                    myTokenOnSale[_isERC721][i].quantity -= _quantity;
                }
            }
            if (
                tokenOnSaleInfo[_isERC721][_tokenId][_sellerAddress].quantity ==
    ...
    function cancelSale(uint256 _tokenId, bool _isERC721) external {
        ...
        for (uint256 i; i < myTokenOnSale[_isERC721].length; i++) {
            if (myTokenOnSale[_isERC721][i].tokenId == _tokenId) {
                delete myTokenOnSale[_isERC721][i];
            }
        }
    ...
    function RmyTokenOnSale(
        bool _isERC721
    ) external view returns (tokenOnSale[] memory) {
        tokenOnSale[] memory myTokens = myTokenOnSale[_isERC721];
        return myTokens;
    }
    ...
    function mint(
        ...
        require(_quantity > 0, "MarketPlace: quantity zero");
        require(checkURI(_uri), "MarketPlace: uri already exist");
        if (_isERC721) {
            IToken721(erc721).mint(to, _uri);
            isTokenMinted[_tokenId] = true;
        } else {
            IToken1155(erc1155).mint(to, _tokenId, _quantity, _uri);
            isTokenMinted[_tokenId] = true;
        }
    }
    ...
    function checkURI(string memory uri) public view returns (bool) {
        if (
            (IToken721(erc721).checkURI(uri)) ||
            (IToken1155(erc1155).checkURI(uri))
        ) {
            return false;
        } else {
            return true;
    ...
    function removeMyToken(
        ...
        for (uint256 i; i < myTokenOnSale[_ERC721].length; i++) {
            if (
                myTokenOnSale[_ERC721][i].seller == _seller &&
                myTokenOnSale[_ERC721][i].tokenId == _tokenId
            ) {
                delete myTokenOnSale[_ERC721][i];
                return;
            }
        }
    }
